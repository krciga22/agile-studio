...
type Props = {
  ...
  accountID: number;
  onClose: () => void;
  onCreated: (schema: BacklogItemTypeSchemaDto) => void;
};

export default function CreateBacklogItemTypeSchemaModal({isOpen, accountID, onClose, onCreated}: Props) {
  const defaultValue = '';
  ...
  const _handleSubmit = async () => {
    try {
      const postDto: BacklogItemTypeSchemaPostDto = {
        title: title.trim(),
        description: description.trim() || undefined,
        accountID
      };

      const response = await createBacklogItemTypeSchema(postDto);

      toast.success('Backlog Item Type Schema Created', Constants.DEFAULT_TOAST_PROPS);
  ...
  return (
    <div className="create-modal-overlay" onMouseDown={onClose} style={{
      ...
      <div className="create-modal" onMouseDown={e => e.stopPropagation()} style={{
        ...
        <h3 style={{marginTop: 0}}>Create Backlog Item Type Schema</h3>
