...
function BacklogItemTypesPage(props: BacklogItemTypesPageProps) {
  ...
  useEffect(() => {
    if(account){
      Utils.setDocumentTitle(`Backlog Item Types - ${getAccountTitle(account)}`);
    }
  }, [account]);
  ...
  return (
    <div className={"BacklogItemTypePage"}>
...

      {
        isRefreshing === false && account &&
          <div>
              ...
              <BacklogItemTypesDataTable accountId={accountId}></BacklogItemTypesDataTable>
          </div>
      }
    </div>
  )
