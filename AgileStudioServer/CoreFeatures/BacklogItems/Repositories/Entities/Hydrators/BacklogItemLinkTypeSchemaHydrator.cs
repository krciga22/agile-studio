...
namespace AgileStudioServer.CoreFeatures.BacklogItems.Repositories.Entities.Hydrators
{
    public class BacklogItemLinkTypeSchemaHydrator : AbstractEntityHydrator
    {
        ...
        public override bool Supports(Type from, Type to)
        {
            return (
                from == typeof(int) ||
                from == typeof(Services.Models.BacklogItemLinkTypeSchema)
        ...
        public override object Hydrate(object from, Type to, int maxDepth, int depth, IHydrator? referenceHydrator = null)
        {
            ...
            if (from is Services.Models.BacklogItemLinkTypeSchema)
            {
                var model = (Services.Models.BacklogItemLinkTypeSchema)from;
                if (model.ID > 0)
                {
